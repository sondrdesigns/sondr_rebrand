...
const { Heading, MonoText, Field, Button, Card, StickyNote, SondrIcon } = window.SondrDesignsDesignSystem_41b26a;
    ...
    <section style={{ padding: '58px 80px 110px' }}>
      ...
      <div style={{ display: 'flex', gap: 80, marginTop: 56, flexWrap: 'wrap' }}>
        ...
        <div style={{ minWidth: 300, display: 'flex', flexDirection: 'column', gap: 0 }}>
          {CONTACTS.map((c) => (
            <div key={c.label} style={{ display: 'flex', gap: 18, alignItems: 'center', padding: '20px 0', borderTop: '1.5px solid var(--rule-color)' }}>
              <span className="sondr-icon"><SondrIcon name={c.icon} size={26} /></span>
              <div>
                <MonoText muted size="small">{c.label}</MonoText>
                <MonoText style={{ marginTop: 4 }}>{c.value}</MonoText>
              </div>
            </div>
          ))}
          <StickyNote tilt={-5} size={210} style={{ marginTop: 40 }}>
            studio hours — mon to thu, 10&ndash;6. we reply to every note.
          </StickyNote>
        </div>
