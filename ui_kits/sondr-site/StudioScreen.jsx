...
const { Heading, MonoText, PolaroidCard, Tape } = window.SondrDesignsDesignSystem_41b26a;

function StudioScreen() {
  const { swatch } = window.SondrData;
  return (
    <section style={{ padding: '58px 80px 110px' }}>
      ...
      {/* team as a pinned board of polaroids */}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 66, marginTop: 66 }}>
        {TEAM.map((m, i) => (
          <div key={m.name} style={{ position: 'relative', width: 240 }}>
            {i % 2 === 0 && (
            ...
            <PolaroidCard width={240} tilt={[-3, 2, -2, 3][i % 4]} assetBase="../../"
              src={swatch(m.tint)} caption={m.name} />
            <MonoText muted size="small" style={{ textAlign: 'center', marginTop: 10 }}>{m.role}</MonoText>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 60, marginTop: 90, flexWrap: 'wrap', borderTop: '1.5px solid var(--rule-color)', paddingTop: 44 }}>
        {ETHOS.map((e) => (
          <div key={e.t} style={{ maxWidth: 300 }}>
            <Heading level="heading" as="h4" style={{ fontSize: 22 }}>{e.t}</Heading>
            <MonoText muted size="small" style={{ marginTop: 12, lineHeight: 1.7 }}>{e.d}</MonoText>
          </div>
        ))}
      </div>
    </section>
